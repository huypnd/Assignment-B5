...
--1
select ename,sum(DATEDIFF(HOUR,checkin,checkout))timework,(select sum(DATEDIFF(DAY,fromDate,toDate)) from request r where r.eid=e.eid group by r.eid) offtime from TimeSheet t join Employee e on t.eid=e.eid
